...
use ffmpeg::{Ffmpeg, JobMsg, JobSpec, Kind, Media};
use lang::{tr, Key, Lang};
use profiles::{Preset, Profile, REMUX_CONTAINERS, X264_PRESETS};
...
impl App {
    ...
    pub fn parallel_plan(&self, n_jobs: usize) -> (usize, u32) {
        let logical = self.cpu.logical.max(1);
        let n = n_jobs.max(1);
        let workers = if self.workers == 0 {
            self.cpu.auto_workers().min(n)
        } else {
            (self.workers as usize).min(n)
        };
...
fn output_ok(output: &Path, pattern: bool) -> bool {
    if pattern {
        let name = output
            .file_name()
...
fn move_to_dir(src: &Path, dir: &Path) -> std::io::Result<PathBuf> {
...
fn src_kbps(m: &Media) -> Option<u64> {
